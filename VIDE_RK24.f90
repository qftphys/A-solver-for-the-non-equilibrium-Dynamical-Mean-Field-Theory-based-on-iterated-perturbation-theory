...
contains
  ...
  subroutine d_vide_rk4(h,f,f0,ek,q,ker)
    ...
    real(8), dimension(size(f))        :: wt
    integer                            :: n,j,nt,it,i
    real(8)                            :: a,b,c,df1,df2,ker0,ker01,ker02,time,detA
    ...
    !
    !<<DEBUG
    !this seems to cure the problems with the oscillations
    f(2:3) = fexact(2:3)
    !>>DEBUG
    ...
    ! !>>DEBUG
    ! write(40,*)(2.d0-1.d0)*h,abs(df2-df(2))
    ! !<<DEBUG
    df1=ek(3)*f(3) + q(3) + h/3.d0*(ker(3,1)*f(1) + 4.d0*ker(3,2)*f(2) + ker(3,3)*f(3))
    ! !>>DEBUG
    ! write(40,*)(3.d0-1.d0)*h,abs(df1-df(3))
    ! !<<DEBUG
    ...
    do n=4,Nt
       ...
       ! !>>DEBUG
       ! write(40,*)dble(n-1)*h,abs(df1-df(n))
       ! !>>DEBUG
    enddo
  ...
  subroutine c_vide_rk4(h,f,f0,ek,q,ker)
    ...
    ker01= 3.d0*ker0/2.d0 + ker(2,1)
    ker02= 3.d0*ker0      + ker(2,2)
    Alin(1,1) =  2.d0/3.d0*ek(2)*h + h*h/9.d0*ker02 - h*h/9.d0*ker(3,2) - 1.d0
    Alin(1,2) = -h/12.d0*ek(3)     - h*h/18.d0*ker0 - h/12.d0*h/3.d0*ker(3,3)
    Blin(1)   = -(5.d0*h/12.d0*df1 + 2.d0*h/3.d0*q(2) - h/12.d0*q(3) + &
         (1.d0 - h/12.d0*h/3.d0*ker(3,1) + h*h/9.d0*ker01)*f(1) )
    !
    Alin(2,1) =  h*h*4.d0/9.d0*ker(3,2) + 4.d0/3.0*h*ek(2) + 2.d0/9.d0*h*h*ker02
    Alin(2,2) = -h**2/9.d0*ker0 - 1.d0 + ek(3)*h/3.d0 + h*h/9.d0*ker(3,3)
    Blin(2)   = -( h/3.d0*df1 + h/3.d0*q(3) +  4.d0/3.d0*h*q(2) + &
         (1.d0 + h/3d0*h/3.d0*ker(3,1) + 2.d0*h*h/9.d0*ker01)*f(1) )
    detA=Alin(1,1)*Alin(2,2)-Alin(1,2)*Alin(2,1)
    Ainv(1,1)=Alin(2,2)/detA
    Ainv(2,2)=Alin(1,1)/detA
    Ainv(1,2)=-Alin(1,2)/detA
