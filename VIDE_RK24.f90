...
contains
  ...
  subroutine d_vie_rk2(h,f,q,ker)
    ...
    real(8),dimension(size(f))         :: q
    real(8),dimension(size(f),size(f)) :: ker
  ...
  subroutine c_vie_rk2(h,f,q,ker)
    ...
    complex(8),dimension(size(f))         :: q
    complex(8),dimension(size(f),size(f)) :: ker
    real(8), dimension(size(f))           :: wt
    integer                               :: n,j,nt
    complex(8)                            :: a,b  
    Nt=size(f)
  ...
  subroutine d_vide_rk2(h,f,f0,hk,q,ker)
    ...
    real(8),dimension(size(f))        :: hk !linear coupling
    real(8),dimension(size(f))        :: q  !costant term
    real(8),dimension(size(f),size(f)):: ker !kernel
  ...
  subroutine c_vide_rk2(h,f,f0,hk,q,ker)!,hstride)
    real(8),intent(in)                    :: h !time-step
    complex(8),intent(in)                 :: f0 !initial-condition
    complex(8),dimension(:),intent(inout) :: f !unknown function
    complex(8),dimension(size(f))         :: hk !linear coupling
    complex(8),dimension(size(f))         :: q  !costant term
    complex(8),dimension(size(f),size(f)) :: ker !kernel
    real(8), dimension(size(f))           :: wt
    integer                               :: n,j,nt
  ...
  subroutine d_vide_rk4(h,f,f0,hk,q,ker)
    real(8),intent(in)                 :: h !time-step
    real(8),intent(in)                 :: f0 !initial-condition
    real(8),dimension(:),intent(out)   :: f !unknown function
    real(8),dimension(size(f))         :: hk
    real(8),dimension(size(f))         :: q
    real(8),dimension(size(f),size(f)) :: ker
    real(8),dimension(size(f))         :: wt
    integer                            :: n,j,nt,it,i
    ...
    !
    Alin(1,1) =  2.d0/3.d0*hk(2)*h + h/9.d0*ker02*h - h/9.d0*ker(3,2)*h - 1.d0
    Alin(1,2) = -h/12.d0*hk(3)     - h/18.d0*ker0*h - h/12.d0*h/3.d0*ker(3,3)
    Blin(1)   = -(5.d0/12.d0*df1*h + 2.d0/3.d0*q(2)*h - h/12.d0*q(3) + &
         (1.d0 - h/12.d0*h/3.d0*ker(3,1) + h/12.d0*ker01*h)*f(1) )
    !
    Alin(2,1) =  4.d0/9.d0*h*h*ker(3,2) + 4.d0/3.0*h*hk(2) + 2.d0/9.d0*h*h*ker02
    Alin(2,2) = -h**2/9.d0*ker0 - 1.d0 + hk(3)*h/3.d0 + h/3.d0*h/3.d0*ker(3,3)
    ...
    !df2=f'(n-2)=f'(2)
    df2=hk(2)*f(2) + q(2) + h/2.d0*(ker(2,1)*f(1)+ker(2,2)*f(2))
    time=(2.d0-1.d0)*h
    write(40,*)time,abs(f(2)-fexact(2)),abs(df2-df(2))
    write(*,*)time,abs(f(2)-fexact(2)),abs(df2-df(2))
    !df1=f'(n-1)=f'(3)
    df1=hk(3)*f(3) + q(3) + h/3.d0*(ker(3,1)*f(1) + 4.d0*ker(3,2)*f(2) + ker(3,3)*f(3))
    time=(3.d0-1.d0)*h
    ...
    ! A = 1.d0 - hk(2)*h/2.d0 - ker(2,2)*h*h/4.d0
    ! C = ker(2,1)*f(1)*wt(1)
    ! B = f(1) + ( df1 + q(2) )*h/2.d0 + C*h/2.d0
    ! f(2) = B/A
    ! df2= hk(2)*f(2) + q(2) + C + ker(2,2)*f(2)*wt(2)
    ...
    ! f(n) = B/A
    ! df1= hk(n)*f(n) + q(n) + C + ker(n,n)*f(n)*wt(n)
    ! write(40,*)dble(n-1)*h,abs(f(n)-fexact(n)),abs(df1-df(n))
    ! write(*,*)dble(n-1)*h,abs(f(n)-fexact(n)),abs(df1-df(n))
    ! !
    do n=4,Nt
       call get_quadrature_weights(wt(1:n),4);wt=wt*h
       A = 1.d0 - h/3.d0*hk(n) - ker(n,n)*h/3.d0*wt(n)
       C = 0.d0
       do j=1,n-1
          C = C + ker(n,j)*f(j)*wt(j)
       enddo
  ...
  subroutine c_vide_rk4(h,f,f0,hk,q,ker)
    real(8),intent(in)                 :: h !time-step
    complex(8),intent(in)                 :: f0 !initial-condition
    complex(8),dimension(:),intent(out)   :: f !unknown function
    complex(8),dimension(size(f))         :: hk
    complex(8),dimension(size(f))         :: q
    complex(8),dimension(size(f),size(f)) :: ker
    real(8),dimension(size(f))         :: wt
    integer                            :: n,j,nt,it,i
    real(8)                            :: time
    complex(8)                            :: a,b,c,c1,c2,df1,df2,ker0,ker01,ker02,detA
