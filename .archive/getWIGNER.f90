program WIGNER_ROTATION
  ...
  USE DLPLOT
  implicit none
  integer           :: i,j,ik,k,m,ia,ir,irel,iave,loop
  complex(8)        :: delta
  !Wigner variables:
  ...
  read(*,*)loop
  write(char,'(i4)')loop
  SRC="DATAsrc_iter"//trim(adjustl(trim(char)))
  ...
  !Read the functions:
  call read_data(locGless(0:nstep,0:nstep),trim(adjustl(trim(SRC)))//"/locGless_data",nstep)
  call read_data(locGgtr(0:nstep,0:nstep),trim(adjustl(trim(SRC)))//"/locGgtr_data",nstep)
  call read_data(Sless(0:nstep,0:nstep),trim(adjustl(trim(SRC)))//"/Sless_data",nstep)
  call read_data(Sgtr(0:nstep,0:nstep),trim(adjustl(trim(SRC)))//"/Sgtr_data",nstep)
  ...
  do ia=0,nstep
     ...
     call splot("nVSepi.ipt",wr(:),nf_wgn(ia,:),append=TT)
  enddo
