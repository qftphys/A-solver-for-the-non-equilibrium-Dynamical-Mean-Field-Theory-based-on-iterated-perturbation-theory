...
contains
  ...
  subroutine kadanof_baym2Gloc
    ...
    complex(8),dimension(0:nstep,0:nstep) :: locGless_tmp,locGgtr_tmp
    real(8),dimension(0:nstep,Lk)         :: nk_tmp
    logical    :: last
    do i=1,10
       if(mpiID==0)call dump("")
    enddo
    if(mpiID==0)call dump("Entering Kadanoff-Baym")
    if(mpiID==0)call dump("----------------------")
    ...
    locGless_tmp=zero; locGgtr_tmp=zero
    nk_tmp=0.d0
    ...
    !===============================================================
    if(mpiID==0)call dump("Solving KB equations:")
    do ik=mpiID,Lk,mpiSIZE !1,Lk
       if(mpiID==0)call print_bar(ik,Lk)
       Gkless=zero;   Gkgtr=zero

       !Recover Initial Condition for Gk^X=<,>
       call recover_ic(ik)
       do istep=0,nstep-1
       ...
       !get local-GF and local-observables:
       locGless_tmp =locGless_tmp + Gkless(:,:)*wt(ik)
       locGgtr_tmp  =locGgtr_tmp  + Gkgtr(:,:)*wt(ik)
       forall(istep=0:nstep)nk_tmp(istep,ik)=-xi*Gkless(istep,istep)
    enddo
    if(mpiID==0)call close_bar
    if(mpiID==0)call dump("")
    call MPI_REDUCE(locGless_tmp(0:nstep,0:nstep),locGless(0:nstep,0:nstep),(nstep+1)*(nstep+1),MPI_DOUBLE_COMPLEX,MPI_SUM,0,MPI_COMM_WORLD,mpiERR)
    call MPI_REDUCE(locGgtr_tmp(0:nstep,0:nstep),locGgtr(0:nstep,0:nstep),(nstep+1)*(nstep+1),MPI_DOUBLE_COMPLEX,MPI_SUM,0,MPI_COMM_WORLD,mpiERR)
    call MPI_REDUCE(nk_tmp(0:nstep,1:Lk),nk(0:nstep,1:Lk),(nstep+1)*Lk,MPI_DOUBLE_PRECISION,MPI_SUM,0,MPI_COMM_WORLD,mpiERR)
    ...
    !Get all the local/k-dependent observables and print out results:
    !===============================================================
    if(mpiID==0)call get_observables
  ...
  subroutine GFstep(ips,ik,istep)
    ...
    !$OMP END DO
    !$OMP END PARALLEL
    !if(ips==2)write(*,"(A,f12.9,f12.9)")'Ikdiag,nk:',maxval(abs(real(Ikdiag))),ntot!maxval(abs(aimag(Ikdiag)))
    return
  ...
  subroutine get_observables
    ...
    real(8),dimension(0:nstep,Lk)             :: npi,sorted_nk,sorted_npi
    real(8),dimension(0:Nx,0:Ny,0:nstep)      :: nDens
    character(len=3)                          :: char
    !real(8),dimension(0:nstep,Lk)             :: epsipi
    ...
    !call shift_kpoint( transpose(epsikt(1:Lk,0:nstep)), epsipi(0:nstep,1:Lk))


    !Fermi surface:
    forall(i=0:nstep,ik=1:Lk)nDens(ik2ix(ik),ik2iy(ik),i)=nk(i,ik)
    ...
    !===============================================================
    call plot_dislin3D("locGless_t1t2","X/$\Delta t$","Y/$\Delta t$","Z",t(0:nstep)/dt,t(0:nstep)/dt,locGless)
    call plot_dislin3D("locGgtr_t1t2","X/$\Delta t$","Y/$\Delta t$","Z",t(0:nstep)/dt,t(0:nstep)/dt,locGgtr)
  ...
  subroutine build_ic
    ...
    real(8) :: w,np,en,nloc,ntot
    complex(8) :: iw
    complex(8),dimension(2*M) :: fg,Sm,eqSiw
    real(8),dimension(0:M) :: tmpGtau
    if(mpiID==0)call dump("Building initial conditions:")
    nloc=0.d0
    icGkless=zero
    ...
    do ik=1,Lk
       ...
       ntot=-tmpGtau(M) !from G(-tau) = -G(beta-tau)==> G(tau=0-)=-G(beta)
       !G_k^<(0,0)=i*G_k^M(\tau=0-)
       icGkless(ik)=xi*ntot 
       if(mpiID==0)call splot("init_nkVSepsk.ipt",epsik(ik),ntot,append=TT)!write(200,*)epsik(ik),ntot
       nloc=nloc+wt(ik)*fermi(epsik(ik),beta0)
    enddo
    if(mpiID==0)call close_bar
    if(mpiID==0)write(*,"(A,f9.6)")"nloc=",nloc
    return
