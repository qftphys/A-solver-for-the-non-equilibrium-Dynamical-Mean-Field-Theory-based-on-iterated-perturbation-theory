...
module OBSERVABLES
  ...
  !LOCAL:
  use VARS_GLOBAL
  use FUNX_NEQ, only: obtain_Gimp
  implicit none
  private

  complex(8),dimension(:,:),allocatable     :: impGless,impGgtr
...
contains
  ...
  subroutine evaluate_print_observables(loop)
    ...
    !Obtain the IMPURITY Green's functions:
    ...
    ! call plot_3D("nVStimeVSepsk3D","t","$\epsilon$","$n(t,k)$",reduced_epsik,t(0:nstep),transpose(reduced_nk(0:nstep,:)),.true.,s1=1,s2=-1)
    ...
    if(loop==nloop)then
       ...
       call system("if [ ! -d DATAfunx ]; then mkdir DATAfunx; fi")
       call splot("G0less_data",G0less(0:nstep,0:nstep))
       call splot("G0gtr_data",G0gtr(0:nstep,0:nstep))
       call splot("locGless_data",locGless)
       call splot("locGgtr_data",locGgtr)
       call splot("impGless_data",impGless)
       call splot("impGgtr_data",impGgtr)
       call splot("Sless_data",Sless)
       call splot("Sgtr_data",Sgtr)
       call system("mv -v *_data DATAfunx/")
  ...
  subroutine get_plot_quasiWigner_functions
    ...
    call splot("Gless_t.ipt",t(-nstep:nstep),gtless,append=TT)
    call splot("Ggtr_t.ipt",t(-nstep:nstep),gtgtr,append=TT)
    call splot("Gret_t.ipt",t(-nstep:nstep),gtret,append=TT)
    ...
    call splot("Gret_realw.ipt",wr,gfret,append=TT)
