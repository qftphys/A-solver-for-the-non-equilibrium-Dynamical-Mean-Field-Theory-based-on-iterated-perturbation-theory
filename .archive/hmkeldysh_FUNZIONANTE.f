...
c---------------------------------------------------------------------
         ...
         do i=1,n
            ...
            if(iloop.eq.nloop)then
               write(80,*)w,dimag(fg0(i)),real(fg0(i))
               write(90,*)w,dimag(fg(i)),real(fg(i))
               write(11,*)w,A,Ab
            endif
         enddo
...
!--------------------------
         do i=-L,L
            ...
            if(iloop.eq.nloop)then
               ...
               write(12,*)dfloat(i)*dt,dimag(g0t(i)),real(g0t(i))
            endif
         enddo
...
!     get Sigma
     ...
     *              real(sigmatpm(i))
               write(93,*)dfloat(i)*dt,dimag(sigmat(i)),
     *              real(sigmat(i))
            enddo
         endif
...
c     si=dimag(fg(i))/pi
c     sr=real(fg(i))/pi
c     if(sr.lt.0.d0)sr=-sr
c     fg(i)=one*sr+xi*si
            if(iloop.eq.nloop)then
               write(84,*)ome(i),dimag(sigma(i)),real(sigma(i))
               write(85,*)ome(i),dimag(fg0(i)),real(fg0(i))
               write(86,*)ome(i),dimag(fg(i)),real(fg(i))
            endif
         enddo
c     fg(1)=fg(2*L)
...
!-----------------------
         if(iloop.gt.10*nloop)then
            ...
            do i=L+1,2*L
               gg=dimag(fg(i))
               if(abs(ome(i)).lt.5.d0.and.gg.ge.0.d0)then
                  write(10,*)ome(i),gg
               endif
            enddo
            do i=1,L
