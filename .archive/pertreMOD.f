...
!       NB: a G0(L+1:2L) corrisponde Sigma(1,L-1)
		 xs=sf(i-L-1)
...
c***    g0 is Go(omega) 
	   if(iloop.ge.nloop)then
	      do i=1,2*L
		 write(90,*)dfloat(i-L-1)*f,dimag(g0(i)),real(g0(i))
	      enddo
	   endif
	   write(90,*)''
c-------------------------------------
c       FFT from omega to time
 ...
 3	   continue
	   ...
	   do 4 i=1,L
	      fg0(i-1)=tg0(i)	![0->L-1] w<0
 4	   continue

	   do 5 i=L+1,2*L
	      fg0(-2*L-1+i)=tg0(i) ![-L->-1] w>0
 5	   continue
	   if(iloop.ge.nloop)then
	      do i=-L+1,L-1
		 write(91,*)dfloat(i)*dtau,dimag(fg0(i))
	      enddo
	   endif
	   write(91,*)''
c       *** fg0(i) is  Go(t)
...
c       FFT from time to omega 
	   if(iloop.ge.nloop)then
	      do i=1,2*L
		 sf2(i)=sft2(i)
		 write(92,*)dfloat(i-L-1)*dtau,dimag(sf2(i))
	      enddo
	   endif
	   write(92,*)''
	   call four1(sf2,L2,-1)
 ...
 35	   continue
	   if(iloop.ge.nloop)then
	      do i=-L,L-1
		 write(93,*)dfloat(i)*f,dimag(sf(i))
	      enddo
	   endif
	   write(93,*)''
...
c       fort.26 is Im[G(w)]  (impurity problem, no self-consistency, imet=1)
		 ...
		 write(26,*)-(f*float(i)),(dns0(i+L+1))
		 write(25,*)-(f*float(i)),(dns(i+L+1))
		 write(24,*)-(f*float(i)),(dimag(sf(-i)))
		 write(23,*)-(f*float(i)),(dreal(sf(-i)))
c	write(27,*)-(f*float(i)),(dreal(g0(i+L+1)))
c	write(28,*)-(f*float(i)),(dimag(g0(i+L+1)))
	      enddo
...
c       go back to the self consistency loop
 100	continue	
	...
	print*,' fort.25 is Im[G(w)]   (the DOS)'
        print*,' fort.23 is Re[Sigma(w)] '
        print*,' fort.24 is Im[Sigma(w)] '
        print*,' fort.26 is Im[G(w)]  (impurity problem)' 
