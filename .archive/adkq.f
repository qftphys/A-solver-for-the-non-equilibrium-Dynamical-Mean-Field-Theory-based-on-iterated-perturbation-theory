!ADKQkb.  A FORTAN CODE FOR SOLVING THE KADANOFF-BAYM EQUATIONS FOR A    ADKQ0000
!1   HOMOGENEOUS FERMION SYSTEM.  H.S. KOHLER, N.H. KWONG, H.A. YOUSIF.  ADKQ0000
!REF. IN COMP. PHYS. COMMUN. 123 (1999) 123                              ADKQ0000
!The program has three components: the main section, the input data, and the
!include statement section. 
!
!1) The main program (KB3d.F) contains 1381 lines 
!2) The input data   (KB3D.DAT) contains 10 lines 
!3) The include statement section (KB3DINC.F) contains 25 lines
!
!We included an output of the program in the manuscript under section "Test
!Runs".
!******
C  KB3d.F
      program kb3d
...
c
     ...
     x          nfreqc(-idim:idim,0:idim**2),
     x          labelc(-idim:idim,0:idim**2)
c
      do 1 i=-idim,idim
      do 1 j=-idim,idim
      do 1 k=-idim,idim
...
c
     ...
     x                conjg(utkc(iq))*dgla0(iq,it)
  32  continue
  31  continue
      do 33 iq=1,nq
      gre(iq,nt+1,nt)=(-eye+gre(iq,nt,nt))*conjg(utk(iq))+
     x                conjg(utkc(iq))*dgla0(iq,nt)
...
c
     ...
     x          h(nvec,nvec,nvec),h1(nvec,nvec,nvec)
...
c
 ...
 19   format(5x,'options selected:')
  ...
  26  format(5x,'electrons calculations,isy= ',i1)
      ...
      if(isy.eq.2)then
      write(6,28)isy
  28  format(5x,'nuclear calculations
     6, isy= ',i1)
      endif
      if(ifk.eq.1)then
      write(6,29)ifk
...
c
 ...
 110  format(3x,'Time',4x,'K. Energy',3x,'P. Energy',3x,'P. Energy',
     6  3x,'T. Energy',3x,'Density',2x,'Quad. Moment'/25x,'(MF)',7x,
     x  '(Corr.)')
      do 120 n=1,ntimes
...
c     (or not using the a cray computer), it must be deleted.
     ...
     x		array,nvec,nvec,array,nvec,nvec,table,work,isys) 
      return
       end
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
...
c     kb3dinc.f
c
c     include file to be used in kb3d.f
c
      ...
      common /par_elec/ hbar,aindb,eryd,amee,epsb,coul,akp,vcd3d,
     x                  isck,ra,rhb,rgam,rgam1,rgam2
      common /par_nucl/ hbarc,p0,pf
      common /grid_size/  dft,dt,dpz,dp3
      common /grid_label/ ix(nq),jx(nq),kx(nq),
     x               irq(-idim:idim,-idim:idim,-idim:idim)
      ...
      common /output/ oke(ntl),ope_corr(ntl),ope_mf(ntl),ote(ntl),
     x                oden(ntl),oqua(ntl),omega_pl
      common /plot/ nplot,iplot(mplot)

******
C    KB3D.DAT
1       !isys, 1=electron, 2=nuclear
132. 4.2e-3  .067 12.998 0 0.4 !aindb,eryd,amee,epsb,isck,akp
1.2 50.e-3 19.4e-3 6.85 2.1 !.25 30.e-3 6.e-3 6.85 2.1 !ra,rhb,rgam,rgam1,rgam2
2.2 1.4  !p0 PF
0.3 4.0 ! DPZ DFT
10 0.0     ! ntime time0
1 3 1  ! ISTEPF(ISTF)  mod_store ifk
0      ! nplot
1 15 30 45 60 !30 40 50 60
                                                                            ****
