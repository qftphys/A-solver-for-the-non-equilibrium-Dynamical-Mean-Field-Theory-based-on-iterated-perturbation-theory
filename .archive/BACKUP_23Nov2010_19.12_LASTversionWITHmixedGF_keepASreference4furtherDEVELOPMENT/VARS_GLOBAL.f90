...
MODULE VARS_GLOBAL
  ...
  integer           :: Lkreduced!reduced grid dimension
  ...
  logical      :: irdeq,wfftw  !flags to fine tune the calculation
  !                             !irdeq=TT read inputs from equilbrium solution
  !                             !wfftw=TT solve with FFTw iff Efield=0
  ...
  !=========================================================  
  real(8),dimension(:),allocatable   :: densmu,epsimu,epsik,sorted_epsik
  integer,dimension(:),allocatable   :: sorted_ik              
  real(8),dimension(:,:),allocatable :: epsikt

  !Dispersion relations arrays:
  !=========================================================  
  real(8),dimension(:),allocatable   :: eqwr,eqwm,eqt,eqtau
  ...
  namelist/parameters/L,Ltau,Lmu,Lkreduced,emin,eps,irdeq,method,wfftw,plot3D
...
contains
  ...
  subroutine read_input_init(inputFILE) 
    ...
    allocate(wr(2*nstep),wm(2*L),t(-nstep:nstep),e(Lmu),tau(0:Ltau))
    allocate(densmu(Lmu))!Bath density
    ...
    rfmesh = pi/dt/dble(L) ; wini= -rfmesh*dble(L) ; edtau = beta/dble(L)
    allocate(eqwr(2*L),eqwm(2*L),eqt(-L:L),eqtau(0:L))
    call init_wgrid(eqwr,wini,rfmesh)
    call init_tgrid(eqt,dt,L)
    call init_wmgrid(eqwm,beta)
    call init_taugrid(eqtau,edtau)
  ...
  subroutine alloc_memory(char)
    ...
    if(char=='a')then
       ...
       allocate(nk(0:nstep,Lk),icnk(Lk),phi(0:nstep,Lk))
  ...
  subroutine get_epsimu(ebini,epsimu_)
    ...
    do i=1,Lmu
       w=ebini+dble(i)*abs(ebfin-ebini)/dble(Lmu)
       iw=cmplx(w,eps)
       gf=zero
       do ik=1,Lmu
          gf=gf+one/(iw-epsimu(ik))/dble(Lmu)
       enddo
       densmu(i)=-aimag(gf)/pi
    enddo
    call splot("DOSbath.lattice",e,densmu)
