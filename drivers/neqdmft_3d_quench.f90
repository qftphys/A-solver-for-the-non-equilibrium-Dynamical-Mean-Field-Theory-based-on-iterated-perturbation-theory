...
program neqDMFT
  ...
  implicit none
  integer                              :: i,j,ik,itime,iloop
  logical                              :: converged
  character(len=16)                    :: finput
  type(kb_contour_gf)                  :: Sbath
  type(kb_contour_gf)                  :: Gloc,Sigma,Gwf,Gwf_old
  ...
  integer                              :: ix,iy,iz
  real(8)                              :: kx,ky,kz
  real(8),allocatable,dimension(:,:)   :: ik2kvec
  !RESULTS:
  ...
  Lk = Nx*Nx*Nx
  allocate(epsik(Lk),wt(Lk),ik2kvec(Lk,3))
  wt  = 1.d0/dble(Lk)
  ik=0
  do ix=1,Nx
     kx = -pi + dble(ix-1)/dble(Nx)*pi2
     do iy=1,Nx
        ky = -pi + dble(iy-1)/dble(Nx)*pi2
        do iz=1,Nx
           kz = -pi + dble(iz-1)/dble(Nx)*pi2
           ik=ik+1
           ik2kvec(ik,:)=[kx,ky,kz]
           epsik(ik) = -2.d0*ts*(cos(kx)+cos(ky)+cos(kz))
        enddo
     enddo
  enddo
  ...
  call allocate_kb_contour_gf(Gwf_old,cc_params)
  ...
  do itime=2,cc_params%Ntime
     ...
     do while(.not.converged.AND.iloop<nloop)
        ...
        ! if(iloop>1.AND.weigth<1.d0)then
        !    Gwf%ret  = weight*Gwf%ret  + (1.d0-weight)*Gwf_old%ret
        !    Gwf%less = weight*Gwf%less + (1.d0-weight)*Gwf_old%less
        !    Gwf%lmix = weight*Gwf%lmix + (1.d0-weight)*Gwf_old%lmix
        ! endif
        ! Gwf_old = Gwf
     enddo
...
contains
  ...
  function hamk(ik,params) result(hk)
    ...
    ky=ik2kvec(ik,2)
    kz=ik2kvec(ik,3)
    hk  = -2.d0*ts*one*(cos(kx)+cos(ky)+cos(kz))
  end function Hamk
