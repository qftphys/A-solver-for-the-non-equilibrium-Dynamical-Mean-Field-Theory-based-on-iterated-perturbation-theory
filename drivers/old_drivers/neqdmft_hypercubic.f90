...
program neqDMFT
  ...
  epsik = linspace(-5.d0*D,5.d0*D,Lk,mesh=de)
  do i=1,Nx
     wt(i) = dens_hyperc(epsik(i),D)
  enddo
  call splot("DOShyperc.ipt",epsik,wt)
  wt=wt*de
  ...
  call allocate_kb_contour_dgf(G0edge,cc_params)
  ...
  forall(ik=1:Lk)nk(1,ik)=dimag(Gk(ik)%less(1,1))
     ...
     do while(.not.converged.AND.iloop<nloop)
        ...
        do ik=1,Lk
           do i=1,itime
              ham(i)=one*epsik(ik)
           enddo
           call vide_kb_contour_gf(Ham,Sigma,Gk(ik),dGk_old(ik),dGk(ik),cc_params)
        ...
        ! if(iloop>1.AND.weight<1.d0)then
        !    Gwf%ret(itime,:)  = weight*Gwf%ret(itime,:)    + (1.d0-weight)*G0edge%ret
        !    Gwf%less(itime,:) = weight*Gwf%less(itime,:)   + (1.d0-weight)*G0edge%less
        !    Gwf%lmix(itime,0:) = weight*Gwf%lmix(itime,0:) + (1.d0-weight)*G0edge%lmix(0:)
        !    Gwf%less(1:itime-1,itime)=-conjg(Gwf%less(itime,1:itime-1))
        ! endif
        ! G0edge%less = Gwf%less(itime,:)
        ! G0edge%ret = Gwf%ret(itime,:)
        ! G0edge%lmix(0:) = Gwf%lmix(itime,0:)
     enddo
...
contains
  ...
  subroutine init_equilibrium_functions(g0,gk,dgk,g,self,params)
    ...
    Self_gtr      =(xi**3)*U*U*g0%mats(1)*g0%mats(L)*g0%mats(0)
    do j=0,L
    ...
    !   
    do ik=1,Lk 
       call setup_initial_conditions(self,gk(ik),dgk(ik),ik,cc_params)
  ...
  subroutine setup_initial_conditions(Self,Gk,dGk,ik,params)
    ...
    dGk%less(1) = -xi*epsik(ik)*Gk%less(1,1)-&
  ...
  subroutine neq_solve_ipt(G0,Sigma,params)
    ...
    Sigma_gtr=zero
    G0_rmix=zero
    forall(i=1:N,j=1:N,i>=j)G0_gtr(i,j)=G0%less(i,j)+G0%ret(i,j)
    forall(i=1:N,j=1:N,i<j)G0_gtr(i,j)=G0%less(i,j)-conjg(G0%ret(j,i))
    ! do j=1,N
    ...
    ! end do
    forall(i=1:N,itau=0:L)G0_rmix(itau,i)  = conjg(G0%lmix(i,L-itau))
    ! do j=0,L
    !    G0_rmix(j,N)  = conjg(G0%lmix(N,L-j))
    ! enddo
    call splot3d("G0_gtr.plot",params%t(1:N),params%t(1:N),G0_gtr(1:N,1:N))
    call splot3d("G0_rmix.plot",params%tau(0:),params%t(1:N),G0_rmix(0:L,1:N))
    !
