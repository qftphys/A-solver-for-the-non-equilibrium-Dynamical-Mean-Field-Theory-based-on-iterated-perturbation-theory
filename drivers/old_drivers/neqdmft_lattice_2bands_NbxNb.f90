...
program neqDMFT
  ...
  !=====================================================================
  allocate(Sigma(Nb),Gloc(Nb,Nb),Gwf(Nb))
  allocate(Sedge(Nb,Nb),Gedge(Nb,Nb))
  ...
  do ib=1,Nb
     ...
     do jb=1,Nb
        call allocate_kb_contour_dgf(Gedge(ib,jb),cc_params)
        call allocate_kb_contour_gf(Gloc(ib,jb),cc_params)  !Local Green's function
        Gloc(ib,jb) = zero
     enddo
  enddo
  do ik=1,Lk
  ...
  do itime=2,cc_params%Ntime
     ...
     do while(.not.converged.AND.iloop<nloop)
        ...
        do ik=1,Lk
           ...
           !4) get G_{ab,k} = V_k
           do ib=1,Nb
              call convolute_kb_contour_gf(Fk(ik,ib),Gk(ik,3-ib,3-ib),Gk(ik,ib,3-ib),cc_params,dcoeff=dreal(Hamk(ik,ib,3-ib,cc_params))**2)
           enddo
           !
           do ib=1,Nb
              do jb=1,Nb
                 Gedge(ib,jb)%ret(1:itime) = Gedge(ib,jb)%ret(1:itime)  + wt(ik)*Gk(ik,ib,jb)%ret(itime,1:itime)
                 Gedge(ib,jb)%less(1:itime)= Gedge(ib,jb)%less(1:itime) + wt(ik)*Gk(ik,ib,jb)%less(itime,1:itime)
                 Gedge(ib,jb)%lmix(0:)     = Gedge(ib,jb)%lmix(0:)      + wt(ik)*Gk(ik,ib,jb)%lmix(itime,0:)
              enddo
           enddo
        enddo
        do ib=1,Nb
           do jb=1,Nb
              Gloc(ib,jb)%ret(itime,1:itime)   = Gedge(ib,jb)%ret(1:itime)
              Gloc(ib,jb)%less(itime,1:itime)  = Gedge(ib,jb)%less(1:itime)
              Gloc(ib,jb)%lmix(itime,0:)       = Gedge(ib,jb)%lmix(0:)
              Gloc(ib,jb)%less(1:itime-1,itime)=-conjg(Gedge(ib,jb)%less(1:itime-1))
           enddo
        enddo
        ...
        do ib=1,2
           call convolute_kb_contour_gf(Gloc(ib,ib),Sigma(ib),Ker,cc_params,dcoeff=-1.d0)
           call vie_kb_contour_gf(Gloc(ib,ib),Ker,Gwf(ib),cc_params)
        enddo
        !
        !CHECK CONVERGENCE
     ...
     enddo
     call plot_kb_contour_gf("Gloc_l1m1",Gloc(1,1),cc_params)
     call plot_kb_contour_gf("Gloc_l2m2",Gloc(2,2),cc_params)
     call plot_kb_contour_gf("Gloc_l1m2",Gloc(1,2),cc_params)
...
contains
  ...
  subroutine init_equilibrium_functions(g0,gk,dgk,fk,dfk,g,self,params)
    ...
    enddo
    if(.not.g(1,1)%status)stop "init_functions: g is not allocated"
    ...
    do ik=1,Lk 
       ...
       do ib=1,Nb
          do jb=1,Nb
             G(ib,jb)%mats(0:)  = G(ib,jb)%mats(0:)  + wt(ik)*gk(ik,ib,jb)%mats(0:)
             G(ib,jb)%iw(:)     = G(ib,jb)%iw(:)     + wt(ik)*gk(ik,ib,jb)%iw(:)
             G(ib,jb)%ret(1,1)  = G(ib,jb)%ret(1,1)  + wt(ik)*gk(ik,ib,jb)%ret(1,1)
             G(ib,jb)%less(1,1) = G(ib,jb)%less(1,1) + wt(ik)*gk(ik,ib,jb)%less(1,1)
             G(ib,jb)%lmix(1,0:)= G(ib,jb)%lmix(1,0:)+ wt(ik)*gk(ik,ib,jb)%lmix(1,0:)
          enddo
       enddo
    enddo
    return
  ...
  subroutine setup_initial_conditions(Self,Gk,dGk,Fk,dFk,ik,params)
    ...
    do i=1,Lf                   !get G_k(iw) 
       ...
       do ib=1,Nb
          ...
          enddo
          Fk(ib)%iw = one/(xi*params%wm - Hamk(ik,ib,ib,params) - self(ib)%iw)
       enddo
    enddo
    !
    do ib=1,Nb
  ...
  subroutine measure_observables(g,self,sedge,params)
    type(kb_contour_gf)     :: g(:,:)
    type(kb_contour_gf)     :: self(size(g,1))
    type(kb_contour_dgf)    :: sedge(2,size(g,1))    
    type(kb_contour_params) :: params
    integer                 :: unit,itime,ib,Nb,ic
    real(8)                 :: dens(size(g,1)),docc(size(g,1),size(g,1))
  ...
  function measure_dens(g,params) result(dens)
    type(kb_contour_gf)     :: g(:,:)
    type(kb_contour_params) :: params
    real(8)                 :: dens(size(g,1))
    integer                 :: N,ib
    N = params%Nt
    do ib=1,size(g,1)
       dens(ib) = dimag(G(ib,ib)%less(N,N))
    enddo
  ...
  function measure_docc(g,sedge,params) result(docc)
    ...
    do ib=1,Nb
       do k=0,L
          SxG(1,k)=Sedge(ib,1)%lmix(k)*conjg(G(ib,ib)%lmix(N,L-k))
          SxG(2,k)=Sedge(ib,2)%lmix(k)*conjg(G(ib,ib)%lmix(N,L-k))
       end do
       ...
       do k=1,N
          SxG(1,k)=Sedge(ib,1)%ret(k)*G(ib,ib)%less(k,N)
          SxG(2,k)=Sedge(ib,2)%ret(k)*G(ib,ib)%less(k,N)
       end do
       ...
       do k=1,N
          SxG(1,k)=Sedge(ib,1)%less(k)*conjg(G(ib,ib)%ret(N,k))
          SxG(2,k)=Sedge(ib,2)%less(k)*conjg(G(ib,ib)%ret(N,k))
       end do
       if(U/=0)docc(ib,1)=docc(ib,1) + 1.d0/U*params%dt*dimag(kb_trapz(SxG(1,0:),1,N))
       if(Ust/=0)docc(ib,2)=docc(ib,2) + 0.5d0/Ust*params%dt*dimag(kb_trapz(SxG(2,0:),1,N))
    enddo
  ...
  function measure_ekin(g,self,params) result(ekin)
    ...
    if(N==1)then
       do ib=1,Nb
          do k=0,L
             Ker(k)=G(ib,ib)%mats(L-k)*G(ib,ib)%mats(k)
          end do
       ...
       do ib=1,Nb
          do k=0,L
             Ker(k)=G(ib,ib)%lmix(N,k)*conjg(G(ib,ib)%lmix(N,L-k))
          end do
          ekin_(ib)=2.d0*params%dtau*dimag( (-xi)*kb_trapz(Ker(0:),0,L) )
          do k=1,N
             Ker(k)=G(ib,ib)%ret(N,k)*G(ib,ib)%less(k,N)
          end do
          ekin_(ib)=ekin_(ib) + 2.d0*params%dt*dimag(kb_trapz(Ker(0:),1,N))
          do k=1,N
             Ker(k)=G(ib,ib)%less(N,k)*conjg(G(ib,ib)%ret(N,k))
          end do
          ekin_(ib)=ekin_(ib) + 2.d0*params%dt*dimag(kb_trapz(Ker(0:),1,N))
       enddo
    endif
  ...
  function measure_epot(g,sedge,params) result(epot)
    ...
    type(kb_contour_dgf)     :: sedge(2,size(g,1))
    type(kb_contour_params)             :: params
    real(8)                             :: epot,docc(size(g,1),size(g,1)),nt(size(g,1))
    integer                             :: i,k,j,N,L,ib,Nb
    N = params%Nt
    L = params%Ntau
    !
