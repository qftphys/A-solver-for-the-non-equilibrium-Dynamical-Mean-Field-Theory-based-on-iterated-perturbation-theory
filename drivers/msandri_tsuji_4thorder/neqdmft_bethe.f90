...
!    complex(8),dimension(:),allocatable  :: inta, intb
    ...
    !allocate(inta(0:max(L,N)))
    !allocate(intb(0:max(L,N)))

    do j=1,N!-1
       G0_gtr(N,j)=G0%less(N,j)+G0%ret(N,j)
    end do
    !G0_gtr(N,N)=G0%less(N,N)+2d0*G0%ret(N,N)
    ...
    !Sigma%ret(N,N) = 0.5d0*(Sigma_gtr(N,N) - Sigma%less(N,N))
