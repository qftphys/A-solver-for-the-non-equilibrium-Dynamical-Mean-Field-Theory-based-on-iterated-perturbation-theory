...
contains
  ...
  subroutine neq_solve_ipt(G0,Sigma,params)
    ...
    if(ifourth)then
       ...
       tdiff(1:N)           =  params%dt
       !tdiff(1)             =  0.5d0*tdiff(1) 
       !tdiff(N)             =  0.5d0*tdiff(N) 
       tdiff(N+1:2*N)       = -params%dt
       !tdiff(N+1)           =  0.5d0*tdiff(N+1) 
       !tdiff(2*N)           =  0.5d0*tdiff(2*N) 
       tdiff(2*N+1:2*N+L+1) = -xi*params%dtau
       !tdiff(2*N+1)         =  0.5d0*tdiff(2*N+1) 
       !tdiff(2*N+1+L)       =  0.5d0*tdiff(2*N+1+L) 
          ...
          do j=1,Ntot
             if (j.ne.N .and. j.ne.2*N .and. j.ne.L) then
                ...
                do ia=1,Ntot
                   ...
                   do ib=1,Ntot
                      intb = intb + Utime(i)*Utime(j)*Utime(ia)*Utime(ib)* &
                           G0mat(i,ia)*G0mat(ia,ib)*G0mat(ib,j)*G2mat(i,ib)*G2mat(ia,j)*tdiff(ib)
                   enddo
                   int_ij = int_ij + intb*tdiff(ia)
                enddo
                Smat(i,j) = int_ij
