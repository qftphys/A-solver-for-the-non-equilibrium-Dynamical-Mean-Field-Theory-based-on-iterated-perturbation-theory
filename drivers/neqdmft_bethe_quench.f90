...
program neqDMFT
  ...
  do ik=1,Lk
     ham = Hk(ik)
     call neq_setup_edependent_initial_conditions(Gwf,dGwf_old,Sigma,Hk(ik),cc_params)
     do itime=2,cc_params%Ntime
        cc_params%Itime = itime
...
contains
  ...
  subroutine init_equilibrium_functions(g0,dg0,g,self,params)
    ...
    inquire(file="G0iw.restart",exist=bool)
    if(bool)then
       write(*,"(A)")"Reading initial G0(iw) from file G0iw.restart"
       unit = free_unit()
       open(unit,file="G0iw.restart",status='old')
       i = file_length("G0iw.restart")
       if(i/=Lf)then
    ...
    Gcoeff  = tail_coeff_g0(0d0,0d0,0d0,0d0)
  ...
  !+-------------------------------------------------------------------+
  subroutine neq_setup_edependent_initial_conditions(Gk,dGk,Self,Hk,params)
    ...
    call fft_gf_iw2tau(Gk%iw,Gk%mats,beta,Gcoeff)        
    Gk%less(1,1) = -xi*Gk%mats(L)
    Gk%ret(1,1)  = -xi
    forall(i=1:L)Gk%lmix(1,i)=-xi*Gk%mats(L-i+1)
    !
    !Derivatives
    allocate(SxG(L))
    !
    ...
    enddo
  end subroutine neq_setup_edependent_initial_conditions
