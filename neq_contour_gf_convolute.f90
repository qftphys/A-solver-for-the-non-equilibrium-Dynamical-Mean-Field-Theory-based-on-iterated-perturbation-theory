...
subroutine convolute_kb_contour_gf_sigma_recursive(G,S,mask,P,params,dcoeff,ccoeff)
  ...
  L   = params%Ntau
  !
  if(Ntot/=Ng+Ns)stop "contour_gf/convolute_kb_contour_sigma_recursive_all: Ntot != Ns + Ng"
  do i=1,Ng
     if(  (.not.G(i)%status) )stop "contour_gf/convolute_kb_contour_sigma_recursive_all: G(i) not allocated"
  enddo
  do i=1,Ns
     if(  (.not.S(i)%status) )stop "contour_gf/convolute_kb_contour_sigma_recursive_all: S(i) not allocated"
  enddo
  if(  (.not.P%status) )stop "contour_gf/convolute_kb_contour_sigma_recursive_all: P not allocated"
  do i=1,Ng
  ...
  do i=2,Ntot
     if(j*mask(i)/=0)then
        print*,"contour_gf/convolute_kb_contour_sigma_recursive_all: mask has 2 consecutives ones at",i
        stop
     endif
     j=mask(i)
  enddo
  ...
  if(mask(1)==0.AND.mask(2)==0)then
     call convolute_kb_contour_gf(G(1),G(2),Kold,params)
  elseif(mask(1)==0.AND.mask(2)==1)then
     call convolute_kb_contour_gf(G(1),S(2),Kold,params)
  elseif(mask(1)==1.AND.mask(2)==0)then
     call convolute_kb_contour_gf(S(1),G(2),Kold,params)
  else
     print*,"contour_gf/convolute_kb_contour_gf_recursive_all: mask(1)==mask(2)==1"
     stop
  endif
  do i=2,Ntot-1
     if(mask(i)==0)then
        call convolute_kb_contour_gf(Kold,G(i),Knew,params)
     elseif(mask(i)==1)then
        call convolute_kb_contour_gf(Kold,S(i),Knew,params)
     else
        print*,"contour_gf/convolute_kb_contour_gf_recursive_all: mask(i)!=1.OR.0"
        stop
     endif
     Kold = Knew
  enddo
  if(mask(Ntot)==0)then
     call convolute_kb_contour_gf(Kold,G(Ntot),P,params)
  elseif(mask(Ntot)==1)then
     call convolute_kb_contour_gf(Kold,S(Ntot),P,params)
  else
     print*,"contour_gf/convolute_kb_contour_gf_recursive_all: mask(Ntot)!=1.OR.0"
     stop
  endif
  !
  if(present(dcoeff))then
...
!C(t,t') = (a_1*a_2*...*a_n)(t,t')with t=t_max && t'=0,t_max; t_max_index==N
! a_i = contour_gf    for i=j_1,...,j_ng
! a_i = contour_scalar for i=k_1,...,k_ns
subroutine convolute_kb_contour_gf_delta_recursive(G,E,mask,P,params,dcoeff,ccoeff)
  type(kb_contour_gf),dimension(:) :: G !array of contour_GF
  complex(8),dimension(:,:)        :: E !array of contour_scalars
  integer,dimension(:)             :: mask !mask array to select contour_GF (0) over contour_scalar (1)
  type(kb_contour_gf)              :: P !result
  type(kb_contour_params)          :: params
  real(8),optional                 :: dcoeff
  complex(8),optional              :: ccoeff
  ...
  L   = params%Ntau
  !
  if(Ntot/=Ng+Ne)stop "contour_gf/convolute_kb_contour_gf_recursive_all: Ntot != Ne + Ng"
  if(size(E,2)<N)stop "contour_gf/convolute_kb_contour_gf_recursive_all: size(E,2) < params%Nt"
  do i=1,Ng
     if(  (.not.G(i)%status))stop "contour_gf/convolute_kb_contour_gf_recursive_all: G(i) not allocated"
  enddo
  if(  (.not.P%status))stop "contour_gf/convolute_kb_contour_gf_recursive_all: P not allocated"
  !
  call allocate_kb_contour_gf(Knew,params)
  call allocate_kb_contour_gf(Kold,params)
  !
  checkP=check_dimension_kb_contour(P,params%Ntime,L)
  do i=1,Ng
     checkG=check_dimension_kb_contour(G(i),params%Ntime,L) 
  enddo
  !check that mask does not contain 2 consecutive ones:
  j=mask(1)
  do i=2,Ntot
     if(j*mask(i)/=0)then
        print*,"contour_gf/convolute_kb_contour_gf_recursive_all: mask has 2 consecutives ones at",i
        stop
     endif
     j=mask(i)
  enddo
  !
  !Perform the recursive convolution:
  !get the first product P1 = g1/e1 * g2/e2 (can not be e1*e2 though)
  !this step features 3 cases: i) g1*g2, ii) g1*e2, iii) e1*g2
  if(mask(1)==0.AND.mask(2)==0)then
     call convolute_kb_contour_gf(G(1),G(2),Kold,params)
  elseif(mask(1)==0.AND.mask(2)==1)then
     call convolute_kb_contour_gf(G(1),E(2,:),Kold,params)
  elseif(mask(1)==1.AND.mask(2)==0)then
     call convolute_kb_contour_gf(E(1,:),G(2),Kold,params)
  else
     print*,"contour_gf/convolute_kb_contour_gf_recursive_all: mask(1)==mask(2)==1"
     stop
  endif
  do i=2,Ntot-1
     if(mask(i)==0)then
        call convolute_kb_contour_gf(Kold,G(i),Knew,params)
     elseif(mask(i)==1)then
        call convolute_kb_contour_gf(Kold,E(i,:),Knew,params)
     else
        print*,"contour_gf/convolute_kb_contour_gf_recursive_all: mask(i)!=1.OR.0"
        stop
     endif
  ...
  if(mask(Ntot)==0)then
     call convolute_kb_contour_gf(Kold,G(Ntot),P,params)
  elseif(mask(Ntot)==1)then
     call convolute_kb_contour_gf(Kold,E(Ntot,:),P,params)
  else
     print*,"contour_gf/convolute_kb_contour_gf_recursive_all: mask(Ntot)!=1.OR.0"
     stop
  endif
  ...
  call deallocate_kb_contour_gf(Kold)
  !    
end subroutine convolute_kb_contour_gf_delta_recursive
...
subroutine convolute_kb_contour_gf_delta_left(A,B,C,params)
  ...
  if(  (.not.B%status).OR.&
       (.not.C%status))stop "contour_gf/convolute_kb_contour_gf: B,C not allocated"
  N   = params%Nt      !<== work with the ACTUAL size of the contour
  L   = params%Ntau
  !
  if(size(A)<N)stop "contour_gf/convolute_kb_contour_gf: size(A) < N"
  checkB=check_dimension_kb_contour(B,params%Ntime,L) 
  checkC=check_dimension_kb_contour(C,params%Ntime,L)
  !
  ! Matsubara
...
subroutine convolute_kb_contour_gf_delta_right(A,B,C,params)
  ...
  logical                             :: checkA,checkC
  if(  (.not.A%status).OR.&
       (.not.C%status))stop "contour_gf/convolute_kb_contour_gf: A,C not allocated"
  N   = params%Nt      !<== work with the ACTUAL size of the contour
  L   = params%Ntau
  !
  checkA=check_dimension_kb_contour(A,params%Ntime,L)
  if(size(B)<N)stop "contour_gf/convolute_kb_contour_gf: size(B) < N"
  checkC=check_dimension_kb_contour(C,params%Ntime,L)
  !
  ! Matsubara
  if (N.eq.1) then
